...
inline float calculateDifficultyResetMultiplier(const uint64_t blockIndex,
                                                ...
                                                const float resetMultiplier)
{
    ...
    float currrentDifficultyMultiplier = (currentDifficultyResetDepthPercent * currentDifficultyReduction)
                                         + resetMultiplier;
...
uint64_t adjustForDifficultyReset(const uint64_t nextDifficulty, const uint64_t blockIndex)
{
    ...
    if (blockIndex >= CryptoNote::parameters::DIFFICULTY_RESET_HEIGHT_V1 &&
        blockIndex < CryptoNote::parameters::DIFFICULTY_RESET_HEIGHT_V1 + CryptoNote::parameters::DIFFICULTY_RESET_WINDOW_V1)
    {
        /* Calculate our adjusted difficulty multiplier */
        float currrentDifficultyMultiplier =
            calculateDifficultyResetMultiplier(blockIndex,
