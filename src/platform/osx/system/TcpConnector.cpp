                        ...
                        if (errno == EINPROGRESS)
                        {
                            ...
                            else
                            {
                                ...
                                dispatcher->getCurrentContext()->interruptProcedure = [&] {
                                    assert(dispatcher != nullptr);
                                    assert(context != nullptr);
                                    ConnectorContext *ctx = static_cast<ConnectorContext *>(context);
                                    if (!ctx->interrupted)
                                    {
                                ...
                                struct kevent disableEvent;
                                EV_SET(&disableEvent, connection, EVFILT_WRITE, EV_ADD | EV_DISABLE, 0, 0, NULL);

                                if (kevent(dispatcher->getKqueue(), &disableEvent, 1, NULL, 0, NULL) == -1)
                                {
                                    message = "kevent failed, " + lastErrorMessage();
                                }
                                else
