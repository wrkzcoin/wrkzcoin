        ...
        for (const auto &input : inputsAndFakes)
        {
...

            {
                std::string dbg = "Ring-sig SIGN input=" + std::to_string(i)
                    + " keyImage=" + Common::podToHex(std::get<CryptoNote::KeyInput>(tx.inputs[i]).keyImage)
                    + " amount=" + std::to_string(std::get<CryptoNote::KeyInput>(tx.inputs[i]).amount)
        ...
        if (!result.error)
        {
            ...
            if (CryptoNote::fromBinaryArray(roundTripped, txBin))
            {
                ...
                for (size_t idx = 0; idx < inputsAndFakes.size(); idx++)
                {
                    ...
                    if (!Crypto::crypto_ops::checkRingSignature(
                            rtPrefixHash,
                            std::get<CryptoNote::KeyInput>(roundTripped.inputs[idx]).keyImage,
                            pks,
