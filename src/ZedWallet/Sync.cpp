                           ...
                           bool fetchTimestamp)
{
    ...
    if (fetchTimestamp)
    {
        ...
        /* Couldn't get timestamp, maybe old node or Wrkzd closed */
        if (blockTime != "")
        {
...
void saveCSV(CryptoNote::WalletGreen &wallet, CryptoNote::INode &node)
{
    ...
    for (size_t i = 0; i < numTransactions; i++)
    {
        ...
        if (t.totalAmount != 0) {
            ...
            if (t.totalAmount < 0)
            {
                /* Put Wrkz in separate field, makes output more usable in spreadsheet */
                std::string splitAmtWRKZ = formatAmount(-t.totalAmount);
                boost::replace_all(splitAmtWRKZ, " ", ",");
                myfile << "-" << splitAmtWRKZ << ",OUT\n";
            }
            else
            {
                std::string splitAmtWRKZ = formatAmount(t.totalAmount);
                boost::replace_all(splitAmtWRKZ, " ", ",");
                myfile << splitAmtWRKZ << ",IN\n";
            }
        }
    }
    myfile.close();
                ...
                std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    {
        std::cout << "Your Wrkzd isn't fully synced yet!" << std::endl
    ...
    while (walletHeight < localHeight)
    {
        ...
        if (tmpWalletHeight == walletHeight)
        {
            ...
            {
                std::string warning =
                    "Syncing may be stuck. Try restarting Wrkzd.\n"
                    "If this persists, visit "
                    "https://wrkz.work/contact for support.";
...
ColouredMsg getPrompt(std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    return InformationMsg("[Wrkz " + shortName + "]: ");
}
